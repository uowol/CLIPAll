...
if __name__ == "__main__":
    ...
    parser.add_argument('--algorithm', type=str, default="CLIP")
    ...
    parser.add_argument('--uda_holdout_fraction', type=float, default=0)    # NOTE: 정해주지 않으면 test_env를 포함함
    ...
    print("LOG:",f"exporting dataset... time is \t{now.strftime('%Y-%m-%d %H:%M:%S')}.")
    ...
    for name, loader in evals:
        if 'in' in name: continue   # NOTE: 용량이 너무 커지는 것을 막기 위해 테스트 데이터만 사용
        algorithm.eval()
        for (x, y), _, __ in loader:
            with torch.no_grad():
                dataset['domain'].append(torch.ones_like(y)*int(name[3]))
                dataset['images'].append(x)
                dataset['labels'].append(y)
    with open(args.output_dir+f'/tsne_dataset_{args.dataset}.pkl', 'wb') as fw:
