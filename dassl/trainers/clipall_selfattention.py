...
class TextEncoder(nn.Module):
    def __init__(self, clip_model, width, heads, output_dim):
        super().__init__()    
        scale = width ** -0.5
        ...
        self.textual_projection = nn.Parameter(clip_model.text_projection.cuda()).requires_grad_(True)
        self.attention = nn.MultiheadAttention(width, heads, dtype=clip_model.dtype)
        self.ln_attention1 = LayerNorm(width)
        self.ln_attention2 = LayerNorm(width)
    ...
    def proj(self, text_features):
        # n_layer, n_prompt, d_model
        x = text_features.permute(1, 0, 2).type(torch.float32)  # n_prompt, n_layer, d_model
        x = self.ln_attention1(x)
        x = x.permute(1, 0, 2)  # n_layer, n_prompt, d_model
        x = self.attention(x, x, x)[0]
        x = x.permute(1, 0, 2)  # n_prompt, n_layer, d_model
        x = self.ln_attention2(x[:,-1,:])   # n_prompt, d_model
        x = x @ self.textual_projection.type(torch.float32)
        return x
...
class ImageEncoder(nn.Module):
    def __init__(self, clip_model, width, heads, output_dim):
        ...
        self.dtype = clip_model.dtype
        self.visual_projection = nn.Parameter(clip_model.visual.proj.cuda()).requires_grad_(True)
        self.attention = nn.MultiheadAttention(width, heads, dtype=clip_model.dtype)
        self.ln_attention1 = LayerNorm(width)
        self.ln_attention2 = LayerNorm(width)
    ...
    def proj(self, image_features):
        ...
        x = image_features.permute(1, 0, 2).type(torch.float32)  # batch_size, n_layer, d_model
        x = self.ln_attention1(x)
        x = x.permute(1, 0, 2)  # n_layer, batch_size, d_model
        x = self.attention(x, x, x)[0]
        x = x.permute(1, 0, 2)  # batch_size, n_layer, d_model
        x = self.ln_attention2(x[:,-1,:])   # batch_size, d_model
        x = x @ self.visual_projection.type(torch.float32)
        return x


def _proj_text_features(text_features, text_encoder, device):
    text_embeddings = []
    # n_class, 12, n_prompt, 512
    for text_features_ in text_features:    # class 개수만큼 반복
        # 12, n_prompt, 512
        text_embedding = text_encoder.proj(text_features_) # n_prompt, 512
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        text_encoder = TextEncoder(clip_model, text_width, heads, output_dim)
        if self.dtype == torch.float16:
            text_encoder = text_encoder.cuda()

        image_encoder = ImageEncoder(clip_model, image_width, heads, output_dim)
        if self.dtype == torch.float16:
