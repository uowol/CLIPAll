...
class TextEncoder(nn.Module):
    ...
    def cross_attention(self, text_features, image_features):
        ...
        Q = image_features                  # batch_size, d_model
        # print(f"LOG::text_Q.shape: {Q.shape}")
        K = self.K(x).permute(0, 2, 1)      # n_prompt, d_model, n_layer 
        # print(f"LOG::text_K.shape: {K.shape}")
        V = self.V(x)                       # n_prompt, n_layer, d_model
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        x = self.dropout(x)
        image_features = self.image_encoder.self_attention(x)   # batch_size, 512
        ...
        for t in self.text_features:                            # n_class, 12, n_prompt, 512
            ...
            text_features.append(
                self.text_encoder.cross_attention(
                    t, 
                    image_features.type(torch.float32).to(self.device)
