...
class MLP(nn.Module):
    ...
    def __init__(self, n_inputs, n_outputs):
        ...
        self.input1 = nn.Linear(n_inputs, 512*4)
        self.input2 = nn.Linear(512*4, 512)
...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        self.textual_network = MLP(textual_width*12, output_dim).to(device='cuda', dtype=self.dtype)
        def init_weights(m):
    ...
    def proj(self, text_features, tokenized_prompts, weight):
        ...
        x = text_features.permute(1, 0, 2).view(text_features.shape[1], -1)     # 1, 12 * 512
        x = self.textual_network(x)
        return x
...
class ImageEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        self.dtype = clip_model.dtype
        self.visual_network = MLP(visual_width*12, output_dim).to(device='cuda', dtype=self.dtype)
        def init_weights(m):
            if isinstance(m, nn.Linear):
                torch.nn.init.xavier_uniform(m.weight)
                m.bias.data.fill_(0.01)
    ...
    def proj(self, image_features, weight):
        x = image_features.permute(1, 0, 2).reshape(image_features.shape[1], -1)     # batchsize, 12 * 512
        x = self.visual_network(x)
        return x
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        try:
            ...
            image_features = self.image_encoder.proj(x, custom_image_weight)
