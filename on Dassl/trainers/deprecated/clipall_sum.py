    ...
    for tokens_, base_text_features_ in zip(tokens, base_text_features):
        if clip_model.dtype == torch.float16:
            text_embedding = text_encoder.proj(
                base_text_features_.type(clip_model.dtype), tokens_.cuda()) # 12, 10, 512
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        image_features = image_features.permute(1, 0, 2)    # i_batch_size, i_n_layer, width
        text_features = text_features.permute(1, 0, 2)      # t_batch_size, t_n_layer, width
        
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)

        scores = torch.einsum('abc,dec->adbe', image_features, text_features)   # i_b, t_b, i_n, t_n
        scores = scores.sum(dim=-1)   # i_b, t_b, i_n
        scores = scores.max(dim=-1)[0]   # i_b, t_b
